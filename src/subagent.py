...
task. See docs/subagent-design.md §0, §4.5b.

This module is deliberately self-contained — it does NOT import from
lib_llm_ext, because Agent_10 (the deployment testbed) mounts a slim
overlay lib_llm_ext.py that does not have the fork's full adapter
helpers. The minimal response-cleanup logic below (strip <think>
blocks, strip markdown fences, parse line-leading s-exprs) suffices
for v1 and works under both the overlay and the fork's full
lib_llm_ext shape.

v1 deliberate omissions (documented in docs/subagent-design.md §4.8):
- query, episodes (parent_env_bound — defer parent-state-sharing to v1.1)
- remember, pin, metta, send, delegate (v1_excluded by design)
- Concurrent dispatch (one at a time, synchronously)
- Subagent → subagent recursion
- Subagent cost telemetry surfaced to parent
"""
...
def resolve_or_instantiate_provider(provider_name, model_name, base_url, var_name):
    """Return a dict carrying enough info for _call_subagent_llm to make
    the call. In the fork-form code path we don't reuse module-level
    OpenAI clients (their model and base_url are baked at init);
    instead we build a fresh client per dispatch. This is cheap (no
    network call at construction) and matches the per-persona binding
    intent.

    Returns a dict with keys: client (openai.OpenAI), model, provider_name."""
...
# ----------------------------------------------------------------------
# The portability helper — the ONE function whose body differs between
# fork-form and upstream-form. Upstream port replaces this body with a
# call into the AbstractAIProvider class hierarchy.
# ----------------------------------------------------------------------

def _call_subagent_llm(provider_handle, content, max_tokens):
    """Fork-form body: direct OpenAI-compatible call. Returns the
    response text as a string. Logs to memory/usage.jsonl per the
    existing fork overlay convention so per-dispatch tokens land in
    the same trace as parent calls."""
    client = provider_handle["client"]
...
def _record_usage(model, resp):
    """Append a usage record to memory/usage.jsonl, matching the format
    written by the existing fork overlay. Never raises."""
    try:
        u = getattr(resp, "usage", None)
        if u is None:
            return
        path = os.path.join(
            os.environ.get("MEMORY_DIR", os.path.join(
        ...
        pass  # never let usage tracking interfere with dispatch
