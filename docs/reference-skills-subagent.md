...
| `OMEGACLAW_SUBAGENT_MAX_PERSONA_PROMPT_BYTES` | `262144` | Maximum bytes read from one regular non-symlink persona prompt before optional SHA-256 hashing and prompt construction; symlink/non-regular or oversized prompts fail closed before worker LLM calls and avoid echoing local paths. `0` disables this read cap. |
...
| `OMEGACLAW_SUBAGENT_MAX_LLM_STATE_BYTES` | `65536` | Maximum bytes read from local per-endpoint rate-limit/concurrency JSON state files before resetting malformed or oversized state; state paths are opened as regular non-symlink files so local guard files cannot redirect reads/writes outside the run directory. |
...
| Worker mixes `emit` with other parsed calls or multiple emits, including same-line trailing payload after a quoted `emit` | Structured JSON `status=error` and `EMIT_PROTOCOL_VIOLATION`; transcript status `emit_protocol_violation`. |
